...
def get_selected_node_vector(gs, size):
    result = torch.zeros(size, size, 53)
    for x in range(size):
        for y in range(size):
            node = x+size*y
            v2 = torch.zeros(26, )
            node_x = node % size
            node_y = node // size
            units = (torch.ones((size, size)) - torch.Tensor(gs).permute((2, 0, 1))[10].cpu()).reshape(-1)
            p1_units = gs[:, :, 11]-gs[:, :, 11]*(np.ones((size, size))-gs[:, :, 21])
            if p1_units[y][x] == 0:
                result[y][x] = torch.zeros(53)
            else:
                for pos in range(size * size):
                ...
                result[y][x] = torch.cat((v2, torch.Tensor(gs[node_y][node_x])))
    return result


def get_nodes_vectors(gs, size):
    num_envs = len(gs)
    res = torch.zeros((num_envs, size, size, 53))
    for i in range(num_envs):
        res[i] = get_selected_node_vector(gs[i], size)
    return res.to(device)


class MyThread (threading.Thread):
    def __init__(self, i, gs, size, res):
    ...
    def run(self):
        get_selected_node_vector(self.res, self.id, self.gs, self.size)
...
def get_selected_unit_vector(node, gs, size):
    # v1 = torch.zeros(27, ).to(device)
    v2 = torch.zeros(26, )
    node_x = node % size
    node_y = node // size
    units = (torch.ones((size, size)) - gs.permute((2, 0, 1))[10].cpu()).reshape(-1)
    for pos in range(size * size):
        if pos != node and units[pos] == 1:
            pos_x = pos % size
            pos_y = pos // size
            ...
            # v1 = v1 + w * inf
            v2 = v2 + directions(pos_x, pos_y, node_x, node_y, d, inf)
    return torch.cat((v2, gs[node_y][node_x]))
